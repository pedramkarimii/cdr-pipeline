...
class TestRabbitMQConsumer(unittest.TestCase):
        ...
        MockConnection.assert_called_once_with(
            pika.ConnectionParameters(
                ...
                credentials=pika.PlainCredentials(
                    self.rabbitmq_consumer.username, self.rabbitmq_consumer.password
                ),
    ...
    def test_process_message_valid(self):
        ...
        with patch.object(self.rabbitmq_consumer, "_save_cdr") as mock_save_cdr:
            self.rabbitmq_consumer.process_message(
                MagicMock(), MagicMock(), MagicMock(), json.dumps(valid_message)
            )
            ...
            self.assertEqual(
                saved_cdr["timestamp"],
                parsed_timestamp.replace(tzinfo=timezone.get_current_timezone()),
            )
    ...
    def test_process_message_empty(self):
        ...
        with patch.object(self.rabbitmq_consumer, "_save_cdr") as mock_save_cdr:
            self.rabbitmq_consumer.process_message(
                MagicMock(), MagicMock(), MagicMock(), json.dumps(empty_message)
            )
    ...
    def test_process_message_corrupted_json(self, mock_save_cdr):
        ...
        self.rabbitmq_consumer.process_message(
            MagicMock(), MagicMock(), MagicMock(), corrupted_message
        )
        mock_save_cdr.assert_not_called()
    ...
    def test_process_message_non_json(self):
        ...
        with patch.object(self.rabbitmq_consumer, "_save_cdr") as mock_save_cdr:
            self.rabbitmq_consumer.process_message(
                MagicMock(), MagicMock(), MagicMock(), non_json_message
            )
            mock_save_cdr.assert_not_called()
