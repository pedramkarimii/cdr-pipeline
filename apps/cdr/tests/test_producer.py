...
class TestRabbitMQProducer(unittest.TestCase):
    ...
    def test_publish_message_success(self, mock_connection):
        ...
        mock_connection.return_value.channel.return_value = (
            self.rabbitmq_producer.channel
        )
    ...
    def test_publish_message_retry_logic(self, mock_connection):
        ...
        self.rabbitmq_producer.channel.basic_publish = MagicMock(
            side_effect=pika.exceptions.AMQPConnectionError
        )
        self.rabbitmq_producer.publish_message(message)
        self.assertEqual(
            self.rabbitmq_producer.channel.basic_publish.call_count,
            self.rabbitmq_producer.max_retries,
        )
